...
Rails.application.config.after_initialize do
  Spree.page_builder.themes << Spree::Themes::OneRides
  ...
  # Configure store to support Turkish locale
  if ActiveRecord::Base.connection.table_exists?('spree_stores') && Spree::Store.default.present?
    store = Spree::Store.default
    current_locales = store.supported_locales.to_s.split(',').map(&:strip)
    unless current_locales.include?('tr')
      store.update(supported_locales: (current_locales + ['tr']).compact.uniq.join(','))
    end
  end
